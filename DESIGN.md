...
- 에타 격자는 **월~금 5열** 고정(`EVERYTIME_IMAGE_DAYS`). 세로 시간 범위는 이미지마다 다를 수
  있어(모바일 캡처 8\~18시 / PC 저장 이미지는 저녁까지) **라디오로 선택**: `8~18시` /
  `8~자정` / `직접`. 사용자는 격자 좌상단·우하단 모서리에 두 점만 맞춘다.
- `computeOccupancy(imageData, box, ScanOptions)` 가 box를 가로 5등분, 세로는 방의 슬롯 단위로
  나눈 뒤 방 시간대([start,end])만 잘라낸다. 방이 12시 시작이면 앞부분 버림. 스캔 범위 밖
  (시각·토·일)은 빈 칸으로 두고 수동 편집.
