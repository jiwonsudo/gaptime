...
| 요일 | 기본 월~금 5칸. 방 생성 시 "토·일 포함" 체크하면 7칸 (`day_count` 5 또는 7) |
| 시간 범위 | 시작 6\~22시 / 종료 9\~24시 입력. 기본 8\~18시. **에타 스크린샷은 항상 08\~18시 10칸 고정**이므로, 방 시간대가 이 범위를 벗어난 시간(예: 18\~22시)은 스캔되지 않고 빈 시간으로 두고 수동 편집으로 채운다 |
...
- 사용자는 에타 격자의 **맨 위(8시)·맨 아래(18시)** 모서리에 두 점을 맞춘다. 스크린샷은
  항상 8\~18시 10칸이라 시각 입력은 없다 (`EVERYTIME_IMAGE_START/END` 상수).
- `computeOccupancy` 가 box를 10등분한 뒤 방 시간대([start,end])에 해당하는 행만 잘라낸다.
  방이 12시 시작이면 앞 4칸을 버리고, 18시 넘는 시간은 빈 칸으로 두고 수동 편집.
...
- **최하단 광고 배너** (`AdBanner`): 지금은 `VITE_ADS_ENABLED=1` 일 때만 뜨는 플레이스홀더.
  실제 연결은 Google AdSense(사이트 승인 필요) 또는 쿠팡 파트너스 스크립트.
  방/캘리브레이션 화면 중심부는 건드리지 않고 페이지 맨 아래에만.
- **예상 인원 20명 초과 시 방 생성 100원 결제**: 아직 미구현. 결제(PG 연동: 토스페이먼츠 등)는
  운영 복잡도가 크고, 학교 전체 배포 초반에 결제 장벽은 확산을 막을 수 있음.
  → 우선 광고만으로 운영해보고, 대형 방 비중/서버 비용을 본 뒤 재검토.
